...
import (
	...
	"github.com/itsscb/backend-template/bff/api"
	db "github.com/itsscb/backend-template/bff/db/sqlc"
	"github.com/itsscb/backend-template/bff/gapi"
	"github.com/itsscb/backend-template/bff/gw"
...
func runGatewayServer(config util.Config, store db.Store, swaggerFS http.FileSystem) {
	...
	err = pb.RegisterBackendHandlerServer(ctx, grpcMux, &server.Grpc)
	if err != nil {
